...
int record_UnitClause(Formula *formula, int *literal_status, int *clause_status, int *trail, int *trail_pointer); // 将formula中的单变元子句记录到clause_status和literal_status中; 出现矛盾直接返回-1;
...
int dpll_recursive(Formula *formula, int *literal_status, int *clause_status, int *trail, int *trail_pointer);    // dpll_solve内部调用 递归处理dpll算法
