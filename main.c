        ...
        case '2':
            while (1)
            {
                ...
                case '1':
                    ...
                    if (generate_sudoku(sudoku, category) == RES_TIME_OUT)
                    {
                        printf("Error: Time_Out.\n");
                        getchar();
                        break;
                    }
                    else
                    {
                        printf("The generated Sudoku board is:\n");
                        print_sudoku(sudoku);
                        ...
                        else
                        {
                            ...
                            double time = 0;
                            int select_time = 0;
                            int res = solve_sudoku(sudoku, branch_select_stategy, category, &time, &select_time);
                            if (res == RES_SAT)
                            {
                                printf("The solution of the Sudoku board is:\n");
                                print_sudoku(sudoku);
                                printf("Time: %f\n", time);
                                printf("Selecting branch variable times: %d\n", select_time);
                            }
                            ...
                            {
                                printf("The Sudoku board solving time out.\n");
                            }
                            else if (res == RES_UNSAT)
                            {
                                printf("The Sudoku board has no solution.\n");
                            }
                            getchar();
                        }
                    }
                ...
                case '2':
                    ...
                    else
                    {
                        ...
                        for (int i = 1; i <= count; i++)
                        {
                            ...
                            int select_time = 0;
                            Formula formula;
                            int res = parse_cnf(cnf_filename, &formula);
                            if (res == -1)
                            {
                                printf("Error: Failed to parse CNF file %s.\n", cnf_filename);
                                continue;
                            }
                            int *solution = (int *)malloc((formula.variable_num + 1) * sizeof(int));
                            if (res == -1)
                            ...
                            if (res == RES_UNSAT)
                            {
                                printf("The Sudoku board %d has no solution.\n", i);
                                continue;
                            }
                            else if (res == RES_TIME_OUT)
                            {
                                printf("The Sudoku board %d solving time out.\n", i);
                                continue;
                            }
